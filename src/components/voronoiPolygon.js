...
//<Link to={props.allData.site.url}></Link>
    return <svg className={svgVoronoi} viewBox="0 0 100 100" onClick={() => {if(!props.isAnimating){navigate(props.allData.site.url)}}}>
            <defs>
                {isPageGroupPoly ? 
                    <pattern id={`bg${props.id}`} patternUnits="userSpaceOnUse" x="-50%"width="200%" height="100%">
                        ...
                        <image style={{opacity:0.7, backgroundColor:"black"}} href={props.allData.site.previewImg.childImageSharp.fixed.src} height="100%"/>
            ...
            <polygon className={`${polygon} ${(isPageGroupPoly && props.displayingPageGroup && !props.isAnimating) ? backgroundImage:''}`} fill={`url(#bg${props.id})`} points={getFormattedPoints()}/>
        </svg>
}
